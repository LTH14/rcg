...
class LatentDiffusion(DDPM):
    ...
    def instantiate_pretrained_enc(self, config):
        ...
        if config.params.get("pretrained_enc_pca_path", None) is not None:
            pca = np.load(config.params.pretrained_enc_pca_path, allow_pickle=True).item()
    ...
    def _get_denoise_row_from_list(self, samples, desc='', force_no_decoder_quantization=False):
        ...
        for zd in tqdm(samples, desc=desc):
            denoise_row.append(self.decode_first_stage(zd.cuda(), force_not_quantize=force_no_decoder_quantization))
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.rep_cond:
            with torch.no_grad():
                ...
                # scale to [0, 1] as our get_input scale it to [-1, 1]
                x_normalized = (x + 1) / 2
    ...
    def gen_imgs(self):
        ...
        shape = [ddim_sampler.model.model.diffusion_model.in_channels,
                 ddim_sampler.model.model.diffusion_model.image_size,
                 ddim_sampler.model.model.diffusion_model.image_size]

        if self.rep_cond:
            with self.rdm_sampler.model.ema_scope("Plotting"):
                shape_firststage = [self.rdm_sampler.model.model.diffusion_model.in_channels,
                                    self.rdm_sampler.model.model.diffusion_model.image_size,
                                    self.rdm_sampler.model.model.diffusion_model.image_size]
                cond_firststage = {"class_label": self.rdm_fake_class_label * torch.ones(self.generation_config.params.gen_bsz).cuda().long()}
                cond_firststage = self.rdm_sampler.model.get_learned_conditioning(cond_firststage)

                cond, _ = self.rdm_sampler.sample(self.generation_config.params.rdm_steps, conditioning=cond_firststage,
                                                  batch_size=self.generation_config.params.gen_bsz,
                                                  shape=shape_firststage,
                                                  eta=self.generation_config.params.rdm_eta, verbose=False)
                cond = cond.squeeze(-1).squeeze(-1)
                cond = cond.unsqueeze(1)
        else:
            if self.class_cond:
                cond = {"class_label": torch.randint(0, 1000, (self.generation_config.params.gen_bsz,)).cuda().long()}
            else:
                cond = {"class_label": 0 * torch.ones(self.generation_config.params.gen_bsz).cuda().long()}
            cond = self.get_learned_conditioning(cond)

        with self.ema_scope("Plotting"):
            sampled_latents, _ = ddim_sampler.sample(self.generation_config.params.ldm_steps,
                                                     conditioning=cond,
                                                     batch_size=self.generation_config.params.gen_bsz,
                                                     shape=shape,
                                                     eta=self.generation_config.params.eta,
                                                     verbose=False)
        gen_imgs = self.decode_first_stage(sampled_latents)
