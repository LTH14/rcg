...
def gen_img(model, args, epoch, batch_size=16, log_writer=None, cfg=0.0):
    ...
    save_folder = os.path.join(args.output_dir, "steps{}-eta{}-temp{}-iter{}-cfg{}".format(args.rdm_steps, args.eta,
                                                                                           args.temp, args.num_iter,
                                                                                           cfg))
    if misc.get_rank() == 0:
